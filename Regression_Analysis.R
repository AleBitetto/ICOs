...
library(RStata)
{
  ...
  options("RStata.StataPath" = "\"C:\\Users\\Alessandro Bitetto\\Downloads\\StataCorp Stata 14.2\\StataMP-64\"") # https://downloadbull.com/portable-statacorp-stata-14-2-free-download/
...
final_vars=c('ESGDummy', 'LogDurationDays', 'RatingIcomarks', 'TeamSize', 'AdvisorSize',
             ...
             'CategoryOtherDummy', 'CategorySocialDummy', 'CategoryTechDummy')
...
main_vars=setdiff(final_vars, categ_vars)
...
var_rename = data.frame(OLD = c('SuccessDummy', 'ESGDummy', 'LogDurationDays', 'RatingIcomarks', 'TeamSize', 'AdvisorSize',
                        ...
                        NEW = c('ICOSUCCESS', 'ESGFLAG', 'DURATION', 'RATING', 'TEAMSIZE', 'ADVISORSIZE', 'WHITELIST', 'BOUNTY', 'BONUS', 'MVP', 'PRESALE', 'KYC', 'ERC20',
                                ...
                                'LOGWORDS', 'FINSENTIMENT'), stringsAsFactors = F)


### correlation matrix
{
  corr_vars=c(target_var, main_vars)
  ...
  png('./Latex_Table_Figure/01_corr_mat.png', width = 10, height = 10, units = 'in', res=300)
  ...
  write.table(varPos %>% mutate(Label=corr_vars), './Latex_Table_Figure/01_corr_mat_varlabel.csv', sep = ';', row.names = F, append = F)
}
...
### statistics
{
  cmd = paste0("univar ", paste0(c(target_var, main_vars), collapse = " "))
  oo <- capture.output(stata(cmd, data.in = df_final %>% select(all_of(c(target_var, main_vars))))) %>% remove_newline()
  lines_to_remove = c(1, 2, 4, length(oo))
  stats = STATA_to_dataframe(input_lines = oo, lines_to_remove, "./output.txt")
  var_order = read.csv('./Latex_Table_Figure/01_corr_mat_varlabel.csv', sep=";", stringsAsFactors=FALSE)
  ...
  write.table(final_stats, './Latex_Table_Figure/00_statistics.csv', sep = ';', row.names = F, append = F)
}
...
### run regression
{
  ...
  for (region_var in c('CountryNum', 'RegionNum', 'SubRegionNum')){
    for (cluster in c('No', 'Space', 'Time')){
      ...
      write.table(output_df_final, paste0('./Latex_Table_Figure/03_logit_REGION_', region_var, '_CLUSTER_', cluster, '.csv'), sep = ';', row.names = F, append = F)
  ...
  for (region_var in c('CountryNum', 'RegionNum', 'SubRegionNum')){
    for (cluster in c('No', 'Space', 'Time')){
      ...
      write.table(output_df_final, paste0('./Latex_Table_Figure/02_logit_REGION_', region_var, '_CLUSTER_', cluster, '.csv'), sep = ';', row.names = F, append = F)

      if (paste0(region_var, '_', cluster) == selected){
        selected_df = output_df_final %>%
          select(all_of(c('Variable', as.character(selected_columns))))
        write.table(selected_df, paste0('./Latex_Table_Figure/02_logit_REGION_', region_var, '_CLUSTER_', cluster, ' - baseline USED.csv'), sep = ';', row.names = F, append = F)
        selected_df = output_df_final %>%
          select(all_of(c('Variable', as.character(selected_columns+1))))
        write.table(selected_df, paste0('./Latex_Table_Figure/02_logit_REGION_', region_var, '_CLUSTER_', cluster, ' - all USED.csv'), sep = ';', row.names = F, append = F)
      }
...
### world maps
{
  ...
  png('./Latex_Table_Figure/ESG_count_map.png', width = 13, height = 8, units = 'in', res=300)
  plot(
  ...
  write.table(esg_count_total, './Latex_Table_Figure/ESG_count_map.csv', sep = ';', row.names = F, append = F)
  ...
  for (i in c(1,0)){
    ...
    png(paste0('./Latex_Table_Figure/ESG_count_map_', lab, '.png'), width = 13, height = 8, units = 'in', res=300)
    plot(p)
    dev.off()
  ...
  write.table(esg_count_fail_sucess, './Latex_Table_Figure/ESG_count_map_FailureSuccess.csv', sep = ';', row.names = F, append = F)
  
  esg_count_final = esg_count_total %>%
    left_join(esg_count_fail_sucess, by = "Country") %>%
    filter(`Total ICOs` > 0)
  write.table(esg_count_final, './Latex_Table_Figure/ESG_count_map_TOTAL.csv', sep = ';', row.names = F, append = F)
}
...
png('./Latex_Table_Figure/ESG_over_years.png', width = 18, height = 10, units = 'in', res=300)
par(mar=c(0,0,0,0))
par(oma=c(0,0,0,0))
...
png('./Latex_Table_Figure/ESG_count_category.png', width = 13, height = 8, units = 'in', res=300)
plot(
...
png('./Latex_Table_Figure/Target_distribution.png', width = 11, height = 9, units = 'in', res=300)
plot(
ggplot(df_final %>%
         group_by(SuccessDummy) %>%
         summarise(ICOs=n()) %>%
